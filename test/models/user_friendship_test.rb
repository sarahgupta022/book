...
class UserFriendshipTest < ActiveSupport::TestCase
  ...
  test "that creating a friendship works without raising an exception" do
    assert_nothing_raised do
     UserFriendship.create user: users(:sarah), friend: users(:arvind)
    end
  end
  
  test "that creating a friendship based on user id and friend id works" do
    UserFriendship.create user_id: users(:sarah).id, friend_id: users(:arvind).id
    assert users(:sarah).pending_friends.include?(users(:arvind))
  end
  
  context "a new instance" do
    setup do
      @user_friendship = UserFriendship.new user: users(:sarah), friend: users(:arvind)
    end
  ...
  context "#send_request_email" do
     setup do
      @user_friendship = UserFriendship.create user: users(:sarah), friend: users(:arvind)
  end
 ...
 context "#mutual_friendship" do
    setup do
     UserFriendship.request users(:sarah), users(:ruby)
      @friendship1 = users(:sarah).user_friendships.where(friend_id: users(:ruby).id).first
      @friendship2 = users(:ruby).user_friendships.where(friend_id: users(:sarah).id).first
 end
 ...
 context "#accept_mutual_friendship!" do
   ...
   context "#accept!" do
    setup do
      @user_friendship = UserFriendship.request users(:sarah), users(:arvind)
 end
   ...
   should "send an acceptance email" do
    ...
    should "include the friend in the list of friends" do
      @user_friendship.accept!
      users(:sarah).friends.reload
      assert users(:sarah).friends.include?(users(:arvind))
    end
   ...
   context ".request" do
     should "create two user friendships" do
       assert_difference 'UserFriendship.count', 2 do
         UserFriendship.request(users(:sarah), users(:arvind))
       end
     end
     
     should "send a friend requestemail" do
        assert_difference 'ActionMailer::Base.deliveries.size', 1 do
         UserFriendship.request(users(:sarah), users(:arvind))
         end
     end
   ...
   context "#block!" do
     ...
     should "not allow new requests once blocked" do
       @user_friendship.block!
       uf = UserFriendship.request users(:sarah), users(:arvind)
       assert !uf.save
